...
class QuestionsPage:
    ...
    def delete_question_from_db(self, subject, question_id_to_delete):
        delete_question_query = f"DELETE FROM {subject} WHERE id = %s"
    ...
    def view_questions(self):
        st.title(f"View {self.subject} Questions")
        ...
        try:
            ...
            for q in questions:
                st.write(f"ID: {q['id']}, Question: {q['question_text']}, Options: {q['options']}, Correct Answer: {q['correct_answer']}, Duration: {q['duration']} seconds")
        except mysql.connector.Error as err:
    ...
    def fetch_questions(self):
        try:
            # Ensure self.subject is not None before constructing the query
            if self.subject is not None:
                table_name = f"{self.subject}"
                query = f"SELECT * FROM {table_name}"
                print("Constructed Query:", query)  # Add this line for debugging
...
def main():
    # Retrieve username, admin_id, and subject from command-line arguments
